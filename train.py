...
if __name__ == '__main__':
    ...
    with open('semantic-segmentation\config\config.yaml', 'r') as f:
    ...
    # create dataloaders
    # Assuming get_dataloader is your function to load data
    print("Data path:", config['DATA_PATH'])
    dataloaders = get_dataloader(batch_size=config['BATCH_SIZE'], data_dir=config['DATA_PATH'])

    # dataloaders = get_dataloader(config['DATA_PATH'], batch_size=config['BATCH_SIZE'],
                                # resize_shape=(config['IMG_HEIGHT'], config['IMG_WIDTH']))
    print(f"Number of training samples: {len(dataloaders['train'].dataset)}")
    print(f"Number of validation samples: {len(dataloaders['val'].dataset)}")
